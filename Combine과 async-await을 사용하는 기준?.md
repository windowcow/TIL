...
---
## 요약
1. 원래는 소비자(데이터가 필요한 쪽)가 먼저 값을 요청했다.
> 이를 Pull 모델의 데이터 흐름 방식이라고 함


2. 하지만 반응형 프로그래밍 만든 사람들은 데이터 변경에 따라서 소비자(ex: 뷰컨들)들이 받아서 보도록 만들고 싶어 했다. 즉 데이터가 주도적으로 흐름을 시작하는 흐름을 원한거다.
> 이를 Push 모델의 데이터 흐름 방식이라고 함

3. 데이터가 주도적으로 그 값을 스스로 전파하는 상황인만큼 선언적으로 값을 처리하는 파이프라인을 원했다.
> 함수형 프로그래밍처럼..
4. 하지만 DispatchQueue & Completion Handler를 사용하는 기존의 비동기 코드는 반환값을 갖지 않는다.
> 그것도 생산자가 되어야 하는데 말이다.
5. 그래서 마치 반환값을 가지는 것처럼 만들었다. 
```swift
...
```
이거처럼 말이다.
6. 그리고 나중에 async/await을 따로 만들었다. 
> - 절차적으로 보이는 비동기 코드를 작성하기 위해서 말이다. 이를 통해 그간 엄청 복잡하던 비동기 코드를 절차적으로 작성할 수 있게 되었다. (물론 이 외에도 실행 중인 스레드를 양보해가며 스레드 점유를 하지 않는 등 많은 고급 기능들이 있지만 말입니다.)

# 결론
비동기 코드의 작성 방식으로 Combine / async-await을 고르는 것이 아니고
데이터 흐름과 관련해서 정해야 한다고 느꼈습니다.

Combine을 통해서 데이터가 주도하는 흐름을 만들 수도 있고

async/await를 통해서 복잡한 비동기 코드를 작성할 수 있다

이게 제 결론입니다.

끝은 첫 문단으로 맺겠습니다.
> [!TIP]
> 데이터를 소비하는 쪽에서 데이터를 요청하는 방식이라면 `async/await`만 사용해도 상관없습니다.
> **예시**: 뷰 컨트롤러에서 버튼을 누르면 `issues`를 레포지토리에 요청하고 반환 받아 사용하는 상황.
> 
하지만 데이터가 변경될 때 소비자가 그 변경을 받아서 사용하려면 **Combine**을 이용해야 합니다.
> **예시**: 레포지토리의 `issues`가 변경되었을 때 여러 뷰 컨트롤러에서 테이블 뷰를 리로드하는 상황.
